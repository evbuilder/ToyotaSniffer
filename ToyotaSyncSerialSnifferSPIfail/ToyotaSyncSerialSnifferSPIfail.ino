/* Toyota Sniffer
 * Two UART ports used for capturing both HTM and MTH data from Toyota Gen3prius and similar aged Hybrids.
 * 
 * ESP32 for WIFI extraction
 */
//I/O  Due  Due
//     Port NET   Sniffer
...
void setup() {
  ...
  Serial1.begin(250000);  // pre-initialize USART0
  Serial2.begin(250000);  // pre-initialize USART1
...
volatile uint32_t time_last_rx = 0;
uint8_t messageFinished = false;

/******************* LOOP ***********************/
void loop() {
  uint32_t timeElapsed = micros() - time_last_rx;
  if(timeElapsed > (20*8) && !messageFinished) //20us*8 is one byte at 50kBaud
  {
    messageFinished = true;
    // signal data ready to WiFi chip? or maybe binary out Uart
  }
}
...
// SPI CLK - CAN transceiver is recessive on high input. I think that means our clock needs to idle high (and so does HTM and MTH)
/*-------------------------------------------------------------------------
